...
echo '' > $TARGET/packages.include.tmp.txt
echo '' > $TARGET/packages.exclude.tmp.txt
for i in `find $PROJECT_ROOT_DIR/build/src/main/resources/modules/ -name module.xml` ;  do
  ...
  error "    Module name: $PKG"
  
  echo $PKG >> $TARGET/packages.$DEST.tmp.txt
...
sort $TARGET/packages.include.tmp.txt | uniq > $TARGET/modules.include.txt
sort $TARGET/packages.exclude.tmp.txt | uniq > $TARGET/modules.exclude.txt
...
function convertModuleNameToGroupID() {
  inFile=$1;
  outFile=$2;
...
convertModuleNameToGroupID $TARGET/modules.include.txt $TARGET/groupIDs.in.tmp.txt;
convertModuleNameToGroupID $TARGET/modules.exclude.txt $TARGET/groupIDs.ex.tmp.txt;

###  Blacklisted ->  prepend '#'.
  echo > $TARGET/groupIDs.in.tmp-blist.txt;
  while read -r ARTIFACT ; do
    if grep --quiet "$ARTIFACT" $DIRNAME/artifactsBlacklist.txt ; then echo -n "# " >> $TARGET/groupIDs.in.tmp-blist.txt; fi
    echo "$ARTIFACT" >> $TARGET/groupIDs.in.tmp-blist.txt;
  done < $TARGET/groupIDs.in.tmp.txt;
...
cat $TARGET/groupIDs.in.tmp-blist.txt | sed '/^$/d' | sed 's#.*#    <include>\0</include>#' | sed 's@    <.*#.*@    <!-- \0 -->@'
echo '</dependencySourceIncludes>'
echo '<dependencySourceExcludes>'
cat $TARGET/groupIDs.ex.tmp.txt       | sed '/^$/d' | sed 's#.*#    <exclude>\0</exclude>#'
